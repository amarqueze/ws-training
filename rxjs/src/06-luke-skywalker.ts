...
const runSkyWalker = () =>{
    ...
    getRequest('https://swapi.dev/api/people/1').pipe(
        switchMap( response => zip( of(response), getRequest(response.vehicles[0]) ) ),
        map( array => ({ personaje: array[0], vehicles: array[1] }) )
